...
void CmdMergeFit::set_temporary_directory(const std::string &dir)
{
    temp_dir = dir;
}

void CmdMergeFit::set_run_from(int run_id)
{
    run_from = run_id;
}

void CmdMergeFit::set_tri_mesh_tol(double tol)
{
    tri_tol = tol;
}

void CmdMergeFit::set_quad_element_num(int num)
{
    quad_num = num;
}

void CmdMergeFit::set_magnitude_factor(double mag)
{
    magnitude_factor = mag;
}

void CmdMergeFit::set_angle_tol(double tol)
{
    angle_tol = tol;
}

void CmdMergeFit::set_fitting_tol(double tol)
{
    fit_tol = tol;
}

void CmdMergeFit::set_area_tol(double tol)
{
    area_tol = tol;
}

void CmdMergeFit::set_mirror(const std::string &is_m)
{
    if (is_m == "yes")
      is_mirror = true;
    else
...
void CmdMergeFit::set_local_layer(int n_layer)
{
    local_layer = n_layer;
}

void CmdMergeFit::set_smooth_iteration(int n_itr)
{
    smooth_iter = n_itr;
}

void CmdMergeFit::set_surface_degree(int deg_in)
{
    degree = deg_in;
}

void CmdMergeFit::read_config_file(const std::string &fn)
{
    ifstream fin;
    fin.open(fn);
    if (!fin.is_open())
...
bool CmdMergeFit::file_exist(const std::string &fn) const
{
    bool state = false;
    std::ifstream fin;
    fin.open(fn);
...
int CmdMergeFit::generate_tri_mesh_from_CAD(const std::string &cad, const std::string &tri)
{
    std::cout << "==============================================\n";
...
int CmdMergeFit::fix_tri_mesh(const std::string &tri_obj, const std::string &tri_fix)
{
    std::cout << "==============================================\n";
...
int CmdMergeFit::generate_quad_mesh_from_tri_mesh(const std::string &tri_fix, const std::string &patch_info)
{
    std::cout << "==============================================\n";
    std::cout << "Generating quad mesh from the triangle mesh...\n";

    // 1. load tri mesh
    TriMeshPtr tri_mesh = TriMeshIO().LoadOBJ(tri_fix.c_str());

    // 2. gen quad mesh
    QuadMeshPtr mesh = QuadMeshGenerator().GenMesh(tri_mesh, magnitude_factor, preserve_sharp, preserve_boundary, minimum_cost, adaptive_scale, angle_tol);
    if (mesh == nullptr)
    {
...
int CmdMergeFit::fit_NURBS_from_quad_patch(const std::string &patch_info, const std::string &surf)
{
    std::cout << "==============================================\n";
...
int CmdMergeFit::write_CAD_to_step(const std::string &surf_pat, const std::string &cad_out)
{
    std::cout << "==============================================\n";
...
int CmdMergeFit::run(const std::string& cad_file_in, const std::string& cad_file_out, const std::string& config_file)
{
...
int CmdMergeFit::run(const std::string& cad_file_in, const std::string& cad_file_out)
{
    // temporary files
    const std::string tri_obj = temp_dir + "/tri.obj";
    const std::string tri_fix = temp_dir + "/tri_fix.obj";
